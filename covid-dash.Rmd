...
df_chart <- df_chart %>%
  ...
  mutate(chosen_name = ifelse(input$linevar == "Peak value", peak_name, last_name))
...
rnge <- max(df_chart$date) - min(df_chart$date)
...
#            size=1.5,color="#F8766D", alpha=1)+
  ...
  geom_label_repel(                                            # Labe for COVID-19
    data=subset(df_chart, date==ifelse(rnge < 31, max(date), min(date)+30)) ,
    force = 1,
    point.padding = 0,label.padding = 0.5,
    nudge_x = -10,
