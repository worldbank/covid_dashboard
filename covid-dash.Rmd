...
p <- df_chart %>% 
  ...
  geom_line(aes(x=date, y=peak_pmr),                    # show the horizontal line for the cause of death that is closest from below to the peak value of the 7-day average
  ...
  geom_label_repel(                                              # Label for peak cause
    ...
    aes(x=date, y=peak_pmr,label=ifelse(peak_rank==0, "All Causes", paste0("Cause #", peak_rank))),
    color="#F8766D",
    label.size=1.2,
    hjust=0.5, alpha=1,size=5) +
  
    geom_label_repel(                                            # Label for last cause
    data=subset(df_chart, date=="2020-08-01"),
    force = 1,
    point.padding = 0,label.padding = 0.5,
    aes(x=date, y=last_pmr,label=ifelse(last_rank==0, "All Causes", paste0("Cause #", last_rank))),
    color="#F8766D",
    label.size=1.2,
    hjust=0.5, alpha=1,size=5) +
  ...
  theme(
    ...
    strip.text.x = element_text(size = 9, color = "black", vjust=2),
    strip.text.y = element_text(size = 9, color = "black", vjust=2),
...
df_chart$labelcause <- ifelse(df_chart$peak_rank==df_chart$last_rank, 
          ...
          # the 0 rank corresponds to all-cause mortality in 2019
                               ...
                               paste0(df_chart$country2, ": Cause #", df_chart$peak_rank, " = ", df_chart$peak_name)),
                        ifelse(df_chart$peak_rank==0 ,
                               paste0("All Causes and Cause #", df_chart$last_rank, " = ", df_chart$last_name), 
                               paste0(df_chart$country2, ": Cause #", df_chart$peak_rank, " = ", df_chart$peak_name, "; Cause #", df_chart$last_rank, " = ", df_chart$last_name)
                               ))
# chart

top <- plyr::round_any(df_chart$peak_7[1], ifelse(df_chart$peak_7[1]<0.1, 0.01, 0.1),
                       ceiling)  # max range of display on Y axis
df_chart$cumsev7 <- round(100*(df_chart$cumulative_severity_7),1)
df_chart$tooltip2 <- paste0("7-Day Average Severity Ratio: ", df_chart$cumsev7, "%")
num <- median(df_chart$date)
