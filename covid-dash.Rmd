...
covid <- covid$covid
covid_treemap <- covid[!stringr::str_detect(covid, "log scale")]
...
covidlog <- c("COVID-19 Cases: Confirmed (log scale)", "COVID-19 Cases: Deaths (log scale)")
...
output$dynamic2 <- renderUI({
  if (input$tab == "COVID-19 Cases: Confirmed")
  {
    shinycssloaders::withSpinner(
      ggiraphOutput("map1", width = "100%", height = "100%")
    )
  } else {
    shinycssloaders::withSpinner(
      ggiraphOutput("map2", width = "100%", height = "100%")
    )
  }
})

# Loading global shapefile converted into rda
area_df <- pin_get("area-df", board = "rsconnect")

# Map over daily confirmed
output$map1 <- renderggiraph({
  
  g <- plot_cog_map(mapdf = area_df,
               cgdf = cg_data,
               cg_x = longitude_newconfirmed_rol,
               cg_y = latitude_newconfirmed_rol,
               cg_size = total_newconfirmed,
               cg_color = date,
               cg_tooltip = total_newconfirmed,
               title = "Center of Gravity of COVID-19 Cases",
               paragraph = "This is a short paragraph explaining how to read a center of gravity map. 
               Blah blah blah blah blah Blah blah blah blah blah Blah blah blah blah blahBlah blah blah blah blah."
               )
...
output$map2 <- renderggiraph({

  g <- plot_cog_map(mapdf = area_df,
               cgdf = cg_data,
               cg_x = longitude_newdeaths_rol,
               cg_y = latitude_newdeaths_rol,
               cg_size = total_newdeaths,
               cg_color = date,
               cg_tooltip = total_newdeaths,
               title = "Center of Gravity of COVID-19 Deaths",
               paragraph = "This is a short paragraph explaining how to read a center of gravity map.
               Blah blah blah blah blah Blah blah blah blah blah Blah blah blah blah blahBlah blah blah blah blah."
               )
