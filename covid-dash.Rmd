...
selectizeInput("Country Name",
                ...
                multiple = TRUE, selected = c("China", "Iran", "Italy", "Spain", "United States"))
...
renderPlotly ({
  ...
  dfs <- dfm[dfm$date == 2019 & dfm$variable %in% input$size, c(1, 6)]
    ...
    a <- list(x = m$mrv2, y = m$mrv,
              text = m$`Short Name`, xref = "x", yref = "y",
              showarrow = TRUE, arrowhead = 0, ax = 20, ay = -40, arrowcolor= toRGB("grey80"),
              font = list(color = "dark grey", size = 14)
              )
    
  plot_ly(data = df4, x = ~mrv2, y = ~mrv) %>%
    add_trace(type = 'scatter', mode = 'markers', color = ~Region, size = ~siz, sizes = c(20, 100),
              opacity = 1,
              hoverinfo = 'text',
    ...
    #         textposition = "top right") %>% 
      layout(legend = list(x = 0, y = -0.2, orientation = 'h'),
             annotations = a,
             xaxis = list(zeroline = FALSE, showline = FALSE, showgrid = T, 
...
output$plot2 <- renderPlotly({
  ...
  #df1 <- dfm[dfm$`Short Name` == "China" & dfm$variable == "Hospital beds (per 1,000 people)",]
  ...
  df2 <- df1[!is.na(df1$value),]
  
    
  dfp <- df2 %>%
    group_by(iso3c) %>%
    filter(date == max(date)) %>%
    mutate(linelab = ifelse(date == max(date), `Short Name`, NA))
  
  p <- list(x = max(dfp$date),  y = ~dfp$value, text = ~str_wrap(dfp$linelab, 6), textposition = "top right",
            xref = "x", yref = "y", color = dfp$iso3c,
            showarrow = T, arrowhead = 0, ax = 30, ay = 0, arrowcolor= toRGB("grey80"),
            font = list(size = 14)
            )
  ...
  add_trace(x = ~date, y = ~value, name = ~`Short Name`, type = 'scatter', mode = 'lines',
      ...
      #add_annotations(x = max(df1$date),  y = ~df1$value, text = ~df1$linelab, textposition = 'top right',
      #         color = df1$iso3c, textfont = list(size = 16), showlegend = FALSE, showarrow = F) %>% 
  layout(legend = list(orientation = 'h'), #annotations = p,
         xaxis = list(title = '', zeroline = FALSE, showline = FALSE, showgrid = T), 
