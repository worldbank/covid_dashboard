...
renderValueBox({
  ...
  de <- paste0("Total cases: ", con)
  valueBox(tags$p(de, style = "font-size: 70%;")
           #, 
           #icon = "fa-users"
  )
})
...
renderValueBox({
  ...
  de <- paste0("New cases (", my_date2, "): ", con)
  valueBox(tags$p(de, style = "font-size: 70%;")
           #, 
           #icon = "fa-users"
  )
  ...
  dfs <- dfm[dfm$date == 2019 & dfm$variable %in% input$size, c(1, 6)]
    ...
    df4 <- df4 %>% 
     ...
     texty <- paste("</br>", df4$`Short Name`,
                 "</br>X: ", round(df4$mrv2, 1), " (Year: ", df4$mry, ")", 
                 "</br>Y: ", round(df4$mrv, 1), " (Year: ", df4$mry, ")")
